...
## Структура
- Файл `cef.asi` должен находится в корневой папке с игрой (собирается как `cef_loader.dll`).
...
## Сборка
Инструкции по кросс-сборке клиента для Windows с macOS/Linux находятся в `docs/build.md`.
...
Для `cef_create_browser`, `cef_create_ext_browser` и `cef_load_url` теперь можно передавать не только обычные URL со схемой (`http://`, `https://`, `data:`, `about:`), но и локальные пути:
- относительный путь вроде `index.html` или `ui/main.html` будет искаться в `<gta_path>/cef/assets/`
- путь вида `cef/assets/index.html` будет резолвиться от корня игры
- абсолютный путь вроде `C:\\Games\\GTA San Andreas\\cef\\assets\\index.html` будет принят только если он реально находится внутри `<gta_path>/cef/assets`
- любые попытки выйти за пределы `<gta_path>/cef/assets`, в том числе через `..` или `file://`, блокируются на клиенте
